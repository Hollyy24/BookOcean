...
async def login(user: LoginForm):
    try:
        ...
        print(f"Error:{error}")
        return JSONResponse(status_code=200, content={"success": False, "Message": str(error)})
...
async def signin(user: SigninForm):
    ...
    try:
        ...
        if member_data == False:
            return JSONResponse(status_code=500, content={"success": False, "Message": "登入失敗"})
...
async def revise(user: UpdateForm, authorization: str = Header(None)):
    ...
    try:
        ...
        if result == False:
            return JSONResponse(status_code=500, content={"success": False, "Message": "失敗"})
...
async def check_status(authorization: str = Header(None)):
    ...
    if member_data is None:
        return JSONResponse(status_code=307, content={"success": False, "message": "未登入系統，拒絕存取"})
...
async def upload_image(file: UploadFile = File(...), authorization: str = Header(None)):
    ...
    if result is True:
        return JSONResponse(cstatus_code=200, ontent={"ok": result})
    if result is False:
